...
func (ctx *Context) CreateProgramWithIL(sources []byte) (*Program, error) {
    cSources := C.CBytes(sources)
    var err C.cl_int
    clProgram := C.clCreateProgramWithIL(ctx.clContext, cSources, C.size_t(len(sources)), &err)
    if err != C.CL_SUCCESS {
        return nil, toError(err)
    }
    if clProgram == nil {
